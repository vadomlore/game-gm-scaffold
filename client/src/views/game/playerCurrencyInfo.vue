<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="80px" label="钻石">
        <template scope="scope">
        <span>{{scope.row.diamond}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="80px" label="金币">
        <template scope="scope">
        <span>{{scope.row.gold}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="80px" label="体力">
        <template scope="scope">
     ...
     <el-table-column class-name="status-col" label="状态" width="90">
      ...
      <el-table-column align="center" label="编辑" width="300px">
        <template scope="scope">
          <el-button size="small" type="success" @click="handleModifyPlayerCurrency(scope.$index, scope.row)">修改玩家道具数量
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form class="small-space" :model="temp" label-position="left" label-width="120px" style='width: 400px; margin-left:50px;'>
        ...
        <el-form-item label="玩家等级">
          <span class="demonstration">{{temp.level}}</span>
        </el-form-item>
        <el-form-item label="增加钻石">
            <el-input v-model="temp.diamond"> </el-input>
        </el-form-item>
        <el-form-item label="增加金币">
            <el-input v-model="temp.gold"> </el-input>
        </el-form-item>
        <el-form-item label="增加玩家体力">
            <el-input v-model="temp.strength"> </el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="modifyCurrency" type="primary">确定修改</el-button>
...
<script>
import { modifyPlayerCurrency, fetchPlayer, getPlayerPidByUid } from '@/api/player'
...
export default {
  ...
  methods: {
    ...
    handleModifyPlayerCurrency(index, row) {
      this.temp = Object.assign({}, row)
      this.temp.gold = row.gold
      this.temp.diamond = row.diamond
      this.temp.strength = row.strength
      this.dialogStatus = 'modifyCurrency'
    ...
    modifyCurrency() {
      ...
      modifyPlayerCurrency(this.temp.pid, this.temp.diamond, this.temp.gold, this.temp.strength).then(response => {
        this.listLoading = false
        if (!this.noticeGMError(response)) {
